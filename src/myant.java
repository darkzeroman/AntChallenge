...
public class myant implements Ant {
	...
	public Action getAction(Surroundings surroundings) {
		...
		if (!initialized)
			this.intialize();
		Action nextMove = Action.move(Direction.WEST);
		updatingMap(surroundings);

		switch (mode) {
		case EXPLORE:
			if (currLocX != 0 && currLocY != 0
		...
		case TOFOOD:
			break;
		case TOHOME:
			nextMove = Action.move(search(map[20][20]));

			break;
		}
		updateCurrLoc(nextMove.getDirection());
		System.out.println("X: " + currLocX + " Y: " + currLocY);
		return nextMove;
	}
	...
	public void updatingMap(Surroundings surroundings) {
		...
		for (int i = 0; i < 4; i++) {
			...
			MapTile mapTile = map[(currLocX + offsets[i][0])][(currLocY + offsets[i][1])];
			mapTile.setLocation((currLocX + offsets[i][0] - origin), (currLocY
					+ offsets[i][1] - origin));
	...
	public Direction search(MapTile target) {
		...
		// if (i == currLocX && j == currLocY)
		// System.out.print("X");
		// if (map[i][j].prev != null)
		// System.out.print(map[i][j].prevDirection());
		// else
		// System.out.print(" ");
		// }
		// System.out.println();
		...
		// if (path.size() > 2)
		if (path.size() > 0)
			return dirForMapTile(path.get(0), map[currLocX][currLocY]);
		else
			return null;
		// else
		// return dirForMapTile();
	...
	public ArrayList<MapTile> findNeighbors(PriorityQueue<MapTile> pq,
		...
		for (int i = 0; i < 4; i++) {
			...
			int xPos = mapTile.x + offsets[i][0] + origin;
			int yPos = mapTile.y + offsets[i][1] + origin;
	...
	public PriorityQueue<MapTile> readyMap() {
		...
		for (int i = 0; i < map.length; i++)
			for (int j = 0; j < map[i].length; j++) {
				...
				if (i == currLocX && j == currLocY)
					map[currLocX][currLocY].distanceFromSource = 0;
				else
