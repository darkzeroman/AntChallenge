...
public class MapOps {
	...
	public static Cell findClosest(MyAnt ant, WorldMap.type goalType) {
		...
		while (!queue.isEmpty()) {
			...
			if (ant.special && ant.mode == MyAnt.Mode.EXPLORE)
	...
	public static ArrayList<Cell> findNeighbors(PriorityQueue<Cell> pq,
		...
		for (int i = 0; i < 4; i++) {
			...
			if (pq == null) {
				if (checkUnexplored && neighborCell.getType() != WorldMap.type.WALL)
				...
				if (checkUnexplored && neighborCell.getType() != WorldMap.type.WALL
					...
					list.add(neighborCell);
						...
						&& ((neighborCell.getType() != WorldMap.type.UNEXPLORED) && (neighborCell.getType() != WorldMap.type.WALL))
						&& (pq.contains(neighborCell)))
					list.add(neighborCell);
			}
